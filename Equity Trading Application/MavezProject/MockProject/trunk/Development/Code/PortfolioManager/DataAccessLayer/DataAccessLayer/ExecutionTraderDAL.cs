...
namespace DataAccessLayer
{
    public class ExecutionTraderDAL:IExecutionTraderDAL
    {
            ...
            public List<Order> GetFilteredOrders(int orderID, int securityID, string orderSide)
            {
                using (EquityTradingDBEntities ctx = new EquityTradingDBEntities())
                {
                    var orders = from order in ctx.Orders.Include("Security").Include("Status")
                                 where order.StatusID == 2 && order.BlockID == null &&
                                 ((orderID != 0) ? order.OrderID == orderID : true)
            ...
            public List<Order> DisplayOrders()
            {
                using (EquityTradingDBEntities ctx = new EquityTradingDBEntities())
                {
                    var orders = from order in ctx.Orders.Include("Status")
            ...
            public List<Order> DisplaySentOrders()
            {
                using (EquityTradingDBEntities ctx = new EquityTradingDBEntities())
                {
                    var openOrders = from order in ctx.Orders.Include("User").Include("Security").Include("Block").Include("Status")
                                     where order.StatusID == 3//2 for open orders
                                     select order;
                    return openOrders.ToList();
                }
            }
            public List<Order> DisplayPartiallyExecutedOrders()
            {
                using (EquityTradingDBEntities ctx = new EquityTradingDBEntities())
                {
                    var openOrders = from order in ctx.Orders.Include("User").Include("Security").Include("Block").Include("Status")
                                     where order.StatusID == 5//2 for open orders
                                     select order;
                    return openOrders.ToList();
                }
            }

            public List<Order> DisplayExpiredOrders()
            {
                using (EquityTradingDBEntities ctx = new EquityTradingDBEntities())
                {
                    var openOrders = from order in ctx.Orders.Include("User").Include("Security").Include("Block").Include("Status")
                                     where order.StatusID == 6//2 for open orders
                                     select order;
                    return openOrders.ToList();
                }
            }
